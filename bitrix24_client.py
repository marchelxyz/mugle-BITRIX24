...
class Bitrix24Client:
    ...
    def get_task_comment_text_multiple_methods(self, task_id: int, message_id: int, chat_id: int = None) -> Optional[str]:
        """
        Получение текста комментария к задаче через рабочий метод im.dialog.messages.get.
        ...
        # Используем только рабочий метод: im.dialog.messages.get
        if not chat_id:
            logger.warning(f"⚠️ Не удалось получить текст комментария: отсутствует chatId для задачи {task_id}")
            return None
        
        try:
            logger.info(f"🔍 Попытка получить текст комментария через im.dialog.messages.get")
            ...
            if result:
                ...
                if isinstance(result, list):
                    ...
                    if len(result) > 0:
                        ...
                        logger.debug(f"⚠️ Метод вернул пустой список")
                        return None
                ...
                if isinstance(result, dict):
                    ...
                    if comment_text:
                        logger.info(f"✅ Успешно: im.dialog.messages.get - получен текст комментария")
                        return str(comment_text)
                    else:
                        logger.debug(f"⚠️ Метод вернул результат, но без текста сообщения")
                        logger.debug(f"   Доступные поля: {list(result.keys())}")
                else:
                    logger.debug(f"⚠️ Метод вернул неожиданный тип результата: {type(result)}")
            else:
                logger.debug(f"❌ Метод вернул None")
        except Exception as e:
            logger.debug(f"❌ Метод вызвал ошибку: {type(e).__name__}: {e}")
        
        logger.warning(f"⚠️ Не удалось получить текст комментария для сообщения {message_id} к задаче {task_id}")
        return None
