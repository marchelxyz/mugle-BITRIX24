...
Bitrix24 обновил систему задач, интегрировав её с системой чатов. Теперь:
...
1. ✅ Bitrix24 интегрировал задачи с системой чатов
2. ✅ Комментарии к задачам теперь являются сообщениями в чатах
3. ✅ Метод `tasks.task.comment.get` больше не работает
4. ✅ Нужно использовать API чатов (`im.message.get`) для получения комментариев
5. ✅ В вебхуках `ID` комментария всегда `"0"`, реальный ID в `MESSAGE_ID`
