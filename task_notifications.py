...
class TaskNotificationService:
    ...
    async def handle_task_comment_event(self, event: str, comment_data: Dict, auth_data: Dict = None):
        ...
        4. Определяет тип комментария (обычный комментарий vs системное уведомление)
        5. Отправляет уведомления только для обычных комментариев зарегистрированным пользователям
        
        ВАЖНО: Разделение типов комментариев:
        - Обычный комментарий: кто-то написал в чат задачи - отправляется уведомление
        - Системное уведомление: связанное с выполнением задачи (просрочена, почти просрочена,
          начал выполнение, изменение статуса и т.д.) - НЕ отправляется, так как уже обрабатывается
          через handle_task_event (ONTASKUPDATE)
        ...
        try:
            ...
            # Проверяем, является ли комментарий системным уведомлением
            # Системные уведомления обрабатываются через handle_task_event (ONTASKUPDATE)
            # и не должны дублироваться через комментарии
            ...
            # Проверяем на уведомление о создании задачи
            ...
            # Проверяем на системные уведомления о выполнении задачи
            # (просрочена, почти просрочена, начал выполнение, изменение статуса и т.д.)
            if comment_text_to_check and self._is_system_notification(comment_text_to_check):
                logger.info(f"⏭️ Пропуск уведомления о комментарии {comment_id_int} к задаче {task_id_int}: это системное уведомление о выполнении задачи (уже обрабатывается через ONTASKUPDATE)")
                logger.debug(f"   Текст системного уведомления: {comment_text_to_check[:200]}...")
                return
    ...
    def _is_system_notification(self, comment_text: str) -> bool:
        """
        Проверка, является ли комментарий системным уведомлением о выполнении задачи
        
        Системные уведомления - это автоматические сообщения Bitrix24, связанные с выполнением задачи:
        ...
        - изменен срок сдачи
        - изменен исполнитель
        - и другие автоматические уведомления
        
        Такие уведомления обрабатываются через handle_task_event (ONTASKUPDATE) и не должны
        дублироваться через комментарии.
        
        Args:
            comment_text: Текст комментария
            
        Returns:
            True если это системное уведомление, False если это обычный комментарий пользователя
        """
        if not comment_text:
            return False
        ...
        # Паттерны для определения системных уведомлений о выполнении задачи:
        ...
        system_patterns = [
            ...
            # Почти просроченные задачи
            r'почти\s+просроч',
            r'приближается\s+дедлайн',
            r'скоро\s+дедлайн',
            
            # Начало выполнения задачи
            r'начал\s+выполн',
            r'приступил\s+к\s+выполн',
            r'взял\s+в\s+работу',
            
            # Изменение статуса (системные уведомления)
            r'статус\s+изменен',
            r'статус\s+задачи\s+изменен',
            r'изменен\s+статус',
            
            # Изменение срока сдачи (системные уведомления)
            r'изменен\s+срок\s+сдачи',
            r'изменен\s+дедлайн',
            r'срок\s+сдачи\s+изменен',
            r'дедлайн\s+изменен',
            
            # Изменение исполнителя (системные уведомления)
            r'изменен\s+исполнитель',
            r'исполнитель\s+изменен',
            r'назначен\s+новый\s+исполнитель',
            
            # Другие системные уведомления Bitrix24
            r'задача\s+обновлена',
            r'обновлена\s+задача',
            r'изменения\s+в\s+задаче',
            
            # Системные сообщения с тегами Bitrix24 (обычно автоматические)
            r'\[timestamp=',
            r'\[user=\d+\].*?\[/user\]',  # Если весь комментарий состоит только из тега пользователя
        ]
        
        # Проверяем паттерны
        for pattern in system_patterns:
            if re.search(pattern, comment_text_lower):
                logger.debug(f"Обнаружено системное уведомление в тексте: {comment_text[:100]}...")
                return True
        
        # Дополнительная проверка: если комментарий состоит только из тегов Bitrix24 без текста,
        # это скорее всего системное уведомление
        # Удаляем все теги Bitrix24 и проверяем, остался ли текст
        text_without_tags = re.sub(r'\[/?[^\]]+\]', '', comment_text_lower)
        text_without_tags = re.sub(r'\s+', ' ', text_without_tags).strip()
        
        # Если после удаления тегов осталось очень мало текста (меньше 10 символов),
        # это скорее всего системное уведомление
        if len(text_without_tags) < 10:
            logger.debug(f"Комментарий содержит только теги Bitrix24, считаем системным уведомлением: {comment_text[:100]}...")
            return True
